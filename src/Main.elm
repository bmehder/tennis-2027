...
import TennisMatch exposing (CompletedMatch, CompletedSet(..), Game(..), Match(..), MatchInProgress, Player(..), PlayerNames, PointScore(..), SetScore, initialMatch, otherPlayer, pointWon, resume, suspend, tiebreakServer)
...
viewInProgress match =
    div [ class "app-shell" ]
        ...
        , inProgressScoreboard match
        , div [ class "point-controls" ]
            [ pointButton PlayerOne match.players.playerOne
            , pointButton PlayerTwo match.players.playerTwo
            ]
...
viewSuspended : MatchInProgress -> Html Msg
viewSuspended match =
    div [ class "app-shell" ]
        [ h1 [] [ text "Elm Tennis 2027" ]
        , inProgressScoreboard match
        , div [ class "suspension-status" ]
            [ span [ class "suspension-label" ]
                [ text "Match suspended" ]
            , button
                [ class "resume-button"
                , onClick ResumeMatch
                ]
...
pointButton : Player -> String -> Html Msg
pointButton player name =
    button
        [ class "point-button"
        , onClick (PointWonBy player)
        ]
...
suspensionButton : Html Msg
suspensionButton =
    button
        [ class "suspension-button"
...
inProgressScoreboard : MatchInProgress -> Html msg
inProgressScoreboard match =
    div [ class "scoreboard" ]
        [ scoreboardHeader
